...
function emitBlessDance(): void {
  if (typeof window === 'undefined') return;
  window.dispatchEvent(new CustomEvent('game:player-bless-dance', { detail: { duration: 1.9 } }));
}

export function tryBlessFriendGrowingPlotV3(plot: PlotRuntime, plotIndex: number): boolean {
...
export function tryBlessFriendBloomPlotV3(plot: PlotRuntime, plotIndex: number): boolean {
