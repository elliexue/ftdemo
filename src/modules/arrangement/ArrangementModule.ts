...
import { finishArrangementCraftV2 as finishArrangementCraft, takeFlowerFromInventoryV2 as takeFlowerFromInventory } from './arrangementLogicV2';

/**
 * 插花台 3D：三格与花田同套程序化花（茎 + 花冠）；完成时金粒喷发。
 */
export class ArrangementModule implements IModule {
  ...
  private readonly onFinishBound = (): void => {
    this.startCraftAnimation();
  };
  ...
  onPointerDown(hits: THREE.Intersection[], _event: PointerEvent): void {
    for (const h of hits) {
      if (hitArrangementSwitch(h.object)) {
        ...
        if (!ok) pushLog('先放满 3 朵花，再按下插花开关。');
        return;
      }
    }
    for (const h of hits) {
      if (hitArrangementStation(h.object)) {
        const selected = gameState.selectedArrangementSeed;
        if (!selected) {
          pushLog('请先在左侧「插花」中选择花束，再点击花架摆放。');
          return;
        }
        const ok = takeFlowerFromInventory(selected);
        if (ok) pushLog(`已向花架放入 ${selected}。`);
        return;
        pushLog('插花台：走到小镇东侧圆毯处，在左侧「插花」面板用背包花材放入三格，再点「完成作品」。');
        return;
      }
    }
  }
  ...
  private tickPromptPulse(_delta: number): void {
  ...
  private tickCraftAnimation(delta: number): void {
    ...
    for (let i = 0; i < a.flowers.length; i++) {
      ...
      const x = THREE.MathUtils.lerp(start.x, a.center.x + spread * (1 - p), p);
    ...
    if (p > 0.72) {
      ...
      const s = 0.9 + Math.sin(a.t * 5) * 0.04;
      a.bouquet.scale.setScalar(s);
    }
    ...
    if (p >= 1) {
      ...
      this.craftAnim = null;
      this.startFinishCelebration();
      this.syncCraftInteractState();
    }
  }

  private startFinishCelebration(): void {
    const scene = this.sceneRef;
    if (!scene) return;
