 ...
 * 收获 / 进入背包。改品质算法只动本文件。
 */
export function tryHarvestPlot(plot: PlotRuntime, plotIndex: number): boolean {
  ...
  const entry: { seedId: SeedId; quality: number; rare?: boolean } = {
    seedId: plot.seedId,
    quality,
  };
  ...
  if (typeof window !== 'undefined') {
    window.dispatchEvent(
      new CustomEvent('game:toast', {
        detail: { message: `收花成功：${SEED_CONFIG[plot.seedId].label}`, kind: 'harvest' },
      }),
    );
  }
  plot.seedId = null;
